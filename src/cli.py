...
def teste_importacao() -> bool:
    ...
    try:
        import src.agents.site_agent
        import src.browser.actions
        import src.utils.llm_client
        import src.workflows.instagram_lead_express
        import src.workflows.lead_qualificacao
        return True
    except Exception as exc:
...
def teste_qualificacao() -> bool:
    ...
    try:
        ...
        return "classificacao" in resultado and resultado["classificacao"] in ["Quente", "Morno", "Frio"]
    except Exception as exc:
...
def demo_qualificacao_lead() -> dict:
    ...
    resultado = lead_qualificacao.qualificar_lead(
        exemplo_lead,
        contexto_nicho="Imobiliaria comercial premium"
    )
...
Exemplos de uso:
  ...
  python -m src.cli agent --site instagram --objetivo "abrir login e clicar em usuario"
    ...
    # Subcomando: workflow
    parser_workflow = subparsers.add_parser("workflow", help="Executar workflow especifico")
    ...
    parser_agent.add_argument("--site", type=str, required=True, help="Site alvo (instagram, etc)")
    parser_agent.add_argument("--objetivo", type=str, required=True, help="Objetivo da automacao")
