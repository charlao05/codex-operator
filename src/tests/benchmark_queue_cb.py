...
def benchmark_sequential_vs_priority_queue():
    ...
    print("-" * 80)
    print(
        f"Sequential: {sequential_time:.2f}s (processed {processed_critical_at} tasks)"
    )
...
def benchmark_circuit_breaker_overhead():
    ...
    print(
        f"Per-request overhead: {(cb_time - baseline_time) / num_requests * 1_000_000:.2f}μs"
    )
...
def benchmark_cascading_failure_prevention():
    ...
    cb = CircuitBreaker(CircuitBreakerConfig(name="protected_api", failure_threshold=5))
    ...
    print("  - Failing fast (~1ms) instead of timeout (~30ms)")
    print("  - Reducing total wait time significantly")
    print("  - Allowing graceful degradation (use fallback)")
