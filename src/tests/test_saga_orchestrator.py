...
class TestSagaExecutionState:
    ...
    def test_execution_duration_after_start(self):
        ...
        exec = SagaExecution(saga_id="test_123", saga_name="test_saga")
        exec.started_at = datetime.utcnow() - timedelta(seconds=5)
...
class TestRetryLogic:
    ...
    def test_retry_on_failure(self, orchestrator):
        ...
        steps = [
            SagaStep("retry_step", action_with_retry, retry_count=3)
        ]
    ...
    def test_max_retries_exceeded(self, orchestrator):
        ...
        steps = [
            SagaStep("fail_step", always_fail, retry_count=2)
        ]
...
class TestCompensation:
    ...
    def test_compensation_order_reversed(self, orchestrator):
        ...
        def action1(ctx): ctx['s1_done'] = True
        def comp1(ctx): compensation_order.append("comp1")
        
        def action2(ctx): ctx['s2_done'] = True
        def comp2(ctx): compensation_order.append("comp2")
