...
class TestCircuitBreakerWithAPIs:
    ...
    def test_gmail_api_with_circuit_breaker(self):
        ...
        protected_send = lambda **kwargs: cb.call(gmail_send, **kwargs)
    ...
    def test_email_api_with_circuit_breaker(self):
        ...
        with pytest.raises(TimeoutError):
            cb.call(send_smtp, recipients=["test@test.com"], subject="Test", body="Body")
        
        assert cb.state == CircuitState.OPEN
        
        # Agora rejeita fast (não chama SMTP)
        result = cb.call(send_smtp, recipients=["test@test.com"], subject="Test", body="Body")
        assert result is None
    ...
    def test_telegram_api_with_circuit_breaker(self):
        ...
        for i in range(2):
            result = cb.call(telegram_send, chat_id="123456", text="Test")
            if i == 1:
    ...
    def test_cascading_failure_prevention(self):
        ...
        gmail_cb = CircuitBreaker(CircuitBreakerConfig(
            name="gmail",
            failure_threshold=2
        ))
        
        fallback_email_cb = CircuitBreaker(CircuitBreakerConfig(
            name="email_fallback",
            failure_threshold=2
        ))
...
class TestCircuitBreakerRecoveryScenarios:
    ...
    def test_graceful_degradation_with_fallback(self):
        ...
        main_api_cb = CircuitBreaker(CircuitBreakerConfig(
            name="main_api",
            failure_threshold=1
        ))
    ...
    def test_partial_failure_doesnt_cascade(self):
        ...
        whatsapp_cb = CircuitBreaker(CircuitBreakerConfig(
            name="whatsapp",
            failure_threshold=1
        ))
