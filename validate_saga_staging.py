...
def run_saga_validation():
    ...
    result1 = orchestrator.execute("booking-001", "create_booking", CREATE_BOOKING_SAGA, booking_context)
    ...
    for step_name, step_exec in result1.step_executions.items():
        print(f"   - {step_name}: ✅ {step_exec.status} ({step_exec.duration_ms:.2f}ms)")
    ...
    booking_saga_with_failure = [
        SagaStep(
            ...
            compensation=lambda ctx: print(f"  [COMPENSATION] Deletando NF {ctx.get('nf_id')}"),
            timeout=5,
            retry_count=0
        ),
        SagaStep(
            name="send_email_WILL_FAIL",
            action=lambda ctx: (_ for _ in ()).throw(Exception("Email service down")),  # Força erro
            compensation=lambda ctx: print(f"  [COMPENSATION] Cancelando email para {ctx.get('client_email')}"),
            timeout=5,
            retry_count=1  # Retry 1 vez antes de falhar
        ),
        SagaStep(
            name="send_whatsapp",
            action=lambda ctx: {"whatsapp_status": "pending"},  # Nunca executa por causa falha anterior
            compensation=lambda ctx: print(f"  [COMPENSATION] Cancelando WhatsApp para {ctx.get('whatsapp')}"),
            timeout=5,
            retry_count=0
        ),
    ]
    ...
    result2 = orchestrator.execute("booking-002", "create_booking_with_failure", booking_saga_with_failure, booking_context2)
    ...
    for step_name, step_exec in result2.step_executions.items():
        status_icon = "✅" if step_exec.status == "SUCCEEDED" else "❌" if step_exec.status == "FAILED" else "⏭️"
    ...
    retry_saga = [
        SagaStep(
            name="flaky_step",
            action=flaky_action,
            compensation=lambda ctx: print(f"  [COMPENSATION] Nada a fazer"),
            timeout=5,
            retry_count=2  # 2 tentativas = 1 falha + 1 retry
        ),
    ]
    
    result3 = orchestrator.execute("booking-003", "retry_test", retry_saga, {"test": "retry"})
    ...
    print(f"   - Resultado: Recuperado após falha!")
