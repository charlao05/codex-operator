...
def main():
    results = {"generated_at": datetime.now(timezone.utc).isoformat(), "templates": []}

    if not EMAILS_TO_SEND:
        print(
            "Nenhum template encontrado em send_wave1_emails.EMAILS_TO_SEND. Abortando geração local."
        )
        return False
