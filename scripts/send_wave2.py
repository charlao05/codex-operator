...
def send_emails(emails, dry_run=True):
    ...
    if dry_run:
        ...
        for idx, e in enumerate(emails, 1):
            ...
            results.append({'index': idx, 'email': e['to'], 'status': 'simulado', 'timestamp': timestamp})
        ...
        for idx, e in enumerate(emails, 1):
            try:
                ...
                sent += 1
                results.append({'index': idx, 'email': e['to'], 'status': 'enviado', 'timestamp': timestamp})
                print(f'[{idx}/{len(emails)}] Enviado: {e["to"]}')
            except Exception as ex:
                results.append({'index': idx, 'email': e['to'], 'status': 'falha', 'erro': str(ex), 'timestamp': timestamp})
