...
def print_monitoring_dashboard():
    ...
    print(
        f"   • Emails abertos: {total_opened}/{total_sent} ({(total_opened / total_sent * 100):.0f}%)"
    )
    print(
        f"   • Respostas recebidas: {total_replied}/{total_sent} ({(total_replied / total_sent * 100):.0f}%)"
    )
    print(
        f"   • Demos agendadas: {total_demos}/{total_sent} ({(total_demos / total_sent * 100):.0f}%)"
    )
    ...
    for i, contact in enumerate(WAVE1_CONTACTS, 1):
        status_emoji = (
            "🟢" if contact["replied"] else "🟡" if contact["opens"] > 0 else "⚪"
        )
        ...
        print(
            f"   Aberturas: {contact['opens']} | Clicks: {contact['clicks']} | Respondeu: {'✅' if contact['replied'] else '❌'}"
        )
